public class DoubleHashTable<K extends Number, V> {
    ...
    public int getSecondaryPrime() { return secondaryPrime; }
    ...
    public DoubleHashTable(int capacity)
    {
        ...
        secondaryPrime = getPrevPrime(capacity);
    }
    ...
    public int hashFunction(K k)
    {
        ...
        //int hashIndex = (scale * key + shift) % capacity; // professor joshi's example
        int hashIndex = (Math.abs(scale * key + shift) % prime) % capacity;
        while (!isAvailable(hashIndex))
        {
            int probe = (secondaryPrime - (key % secondaryPrime));
            hashIndex = (hashIndex + probe) % capacity;
        }
        return hashIndex;
        // could have used a j variable to multiply the probe value
        // instead hashIndex += probe % capacity works
    ...
    public V find(K k)
    {
        ...
        {
            position = (hashIndex + i * (secondaryPrime - (key % secondaryPrime))) % capacity;
    ...
    {
        System.out.println("prime: " + getPrime() + " Capacity: " + capacity + " SecondaryPrime: " + getSecondaryPrime());
        for (int i = 0; i < table.length; i++)
        {
            if (table[i] == null)
